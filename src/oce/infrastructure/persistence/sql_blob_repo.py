...
class SqlBlobRepository(BlobRepository):
    ...
    async def get_paths(self, blob_names: Sequence[str], limit: int | None = None) -> list[str]:
        if not blob_names:
            return []
        stmt = (
    ...
    async def count_paths(self, blob_names: Sequence[str]) -> int:
        ...
        value = await self.session.scalar(
            select(func.count(func.distinct(BlobModel.path))).where(
    ...
    async def get_path_blob_pairs(
        ...
        if not blob_names:
            return []
        rows = await self.session.execute(
            select(BlobModel.path, BlobModel.blob_name)
            .where(BlobModel.blob_name.in_(blob_names))
            .order_by(BlobModel.path, BlobModel.blob_name)
        )
        return [(path, blob_name) for path, blob_name in rows if path]
    ...
    async def get_blob_contents(
        ...
        grouped: dict[str, tuple[str, list[str], int]] = {}
        for blob_name, path, start_line, end_line, content in rows:
            if blob_name not in grouped:
                grouped[blob_name] = (path, [], 0)
            stored_path, parts, last_line = grouped[blob_name]
            lines = content.splitlines()
            overlap = max(0, last_line - start_line + 1)
            parts.extend(lines[overlap:])
            grouped[blob_name] = (stored_path, parts, max(last_line, end_line))
        return {
            blob_name: (path, "\n".join(parts))
            for blob_name, (path, parts, _last_line) in grouped.items()
        }
